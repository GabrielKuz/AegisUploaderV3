...
def get_AIS_Id(ticket_id: str) -> str | None:
...
def quikSrch(ais_id: str, searchTerm: str) -> str | None:
...
def quikAtrbt(ais_id: str, searchTerm: str) -> str | None:
    ticket = get_ticket(ais_id)
...
def get_ticket_id(ais_id: str) -> str | None:
    return quikAtrbt(ais_id, "id")


def is_ticket_archived(ais_id: str) -> bool | None:
    return quikAtrbt(ais_id, "archived")


def ticket_archive_location(ais_id: str) -> str | None:
...
def get_caseCreateDate(ais_id: str) -> str | None:
    return quikSrch(ais_id, "createdate")


def get_caseCloseDate(ais_id: str) -> str | None:
    return quikSrch(ais_id, "closedate")


def get_caseITARstatus(ais_id: str) -> bool | None:
...
def get_caseCompany(ais_id: str) -> str | None:
    return quikSrch(ais_id, "company_name")


def get_caseSQLServer(ais_id: str) -> str | None:
...
def get_pipeline_stage_id(ais_id: str) -> str | None:
    return quikSrch(ais_id, "hs_pipeline_stage")


def get_pipeline_id(ais_id: str) -> str | None:
    return quikSrch(ais_id, "hs_pipeline")


def get_caseTier(ais_id: str) -> str | None:
    return search_pipeline(ais_id, 0)


def get_caseStatus(ais_id: str) -> str | None:
...
def search_pipeline(ais_id: str, operationProtocolNumber: int) -> str | None:
    ...
    try:
        response = api_client.crm.pipelines.pipelines_api.get_all(object_type="tickets")
...
def is_caseExpirable(ais_id: str) -> bool | None:
...
def _scheduler_get_pipelines():
    ...
    if _scheduler_pipeline_cache is None or now - _scheduler_pipeline_cache_time > _SCHEDULER_PIPELINE_CACHE_TTL:
        try:
            ...
            logger.error(f"Failed retrieving HubSpot pipelines: status={getattr(e, 'status', None)} body={getattr(e, 'body', None)}")
            return None
...
def _scheduler_hubspot_search(case_id: str):
    ...
    for attempt in range(retries):
        try:
            return api_client.crm.tickets.search_api.do_search(search_request)
            ...
            if status_code == 429:
                wait = 2 ** (attempt + 1)
                logger.warning(f"HubSpot rate limited for case {case_id}. Retrying in {wait}s...")
                time.sleep(wait)
                continue

            logger.error(f"HubSpot search failed for {case_id}: status={status_code} body={getattr(e, 'body', None)}")
            return None
...
def get_caseStatus_scheduler(case_id: str) -> str | None:
