...
from modules.AuditLogGenerator import auditLog, getCaseIdFromLinkID, getCaseIdFromUploadID, getRegionFromlinkID, getRegionFromUploadID
from modules.auth import User, requireRoles
from modules.models import LinkRecord, StorageRegion, UploadChunk, UploadRecord, UploadSession
from modules.rateLimit import rate_limit
from modules.uploadSchemas import CompleteUploadResponse, ExtendExpirationResponse, MarkForDeletionResponse, StartUploadResponse, UploadChunkResponse, UploadedFileInfo, UploadStatusResponse
...
def _is_deadlock_error(error: Exception) -> bool:
    return isinstance(error, DeadlockDetected) or (isinstance(error, OperationalError) and isinstance(error.orig, DeadlockDetected))
...
async def start_upload(
    ...
    if link_entry.expiration_date is None or link_entry.expiration_date <= datetime.datetime.now(datetime.UTC):  # If the link has expired, mark it as expired and return 410. This is a backup in case the cleanup job hasnt run yet
...
async def upload_file_chunk(
    ...
    existing_chunk = (
        ...
        .filter(  # Check if the chunk has already been uploaded to prevent duplicate uploads and ensure data integrity
    ...
    for attempt in range(max_database_attempts):
        try:
            with db.begin():
                ...
                upload_session.last_activity = datetime.datetime.now(tz=datetime.UTC)
            ...
            db.rollback()
            logger.error(f"Database update failed after Azure write for upload {upload_session_id}, offset {chunk_offset}. Retry is safe.")
            traceback.print_exc()
...
def upload_status(link_uuid: str, upload_token: str, db: Annotated[sqlalchemy.orm.Session, Depends(get_db)]):
    ...
    if upload_session.completed:  # If its completed, the Chunks have been deleted so we just return the full range and size
        ...
        chunks = db.query(UploadChunk.offset, UploadChunk.size).filter(UploadChunk.upload_id == upload_session.upload_id, UploadChunk.uploaded == True).order_by(UploadChunk.offset).all()

        ranges = [[offset, offset + size] for offset, size in chunks]
...
def listFiles(
    ...
    return [
        UploadedFileInfo(
            ...
            expiration_date=((upload.date_uploaded + datetime.timedelta(days=upload.max_days_in_storage)).isoformat() if (upload.date_uploaded and upload.max_days_in_storage is not None) else None),
...
@auditLog(fromParameter="upload_id", parameterToRegionFunction=getRegionFromUploadID, parameterToCaseIdFunction=getCaseIdFromUploadID)
def extendFileExpiration(
    upload_id: str, additional_days: Annotated[int, Query(gt=0, le=365)], current_user: Annotated[User, Depends(requireRoles("Admin", strict=True))], db: Annotated[sqlalchemy.orm.Session, Depends(get_db)]
...
""",
                ...
                "recipients": {
                    "to": [{"address": link_entry.creator, "displayName": link_entry.creator}],
                    "cc": [  # All other users with access
                        {"address": user, "displayName": user} for user in (link_entry.users_with_access or []) if user != link_entry.creator
                    ],
                },
...
def mark_all_for_deletion(link_uuid: str, current_user: Annotated[User, Depends(requireRoles("Admin", strict=True))], db: Annotated[sqlalchemy.orm.Session, Depends(get_db)]):
    ...
    uploads = db.query(UploadRecord).filter(UploadRecord.link_uuid == link_uuid, UploadRecord.for_deletion.is_(False)).all()
