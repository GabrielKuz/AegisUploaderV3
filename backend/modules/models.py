...
import sqlalchemy
from sqlalchemy import JSON, UUID, BigInteger, Boolean, Column, DateTime, ForeignKey, Integer, String, Text, UniqueConstraint, select
...
class UploadSession(Base):
    ...
    __table_args__ = (UniqueConstraint("link_uuid", "blob_name", name="uq_upload_blob_name_per_link"), {"schema": "LinkDB"})
...
class UploadChunk(Base):
    ...
    __table_args__ = (UniqueConstraint("upload_id", "offset", name="uq_upload_chunk_offset"), {"schema": "LinkDB"})
...
class UploadRecord(Base):  # "LinkDB".uploads table
    ...
    timestamp: Mapped[object | None] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
    ...
    date_uploaded = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(UTC))
...
class LinkRecord(Base):  # "LinkDB".links table
    ...
    expiration_date = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(UTC) + AppConstants.LINK_EXPIRATION_TIME)  # 48 hours from creation
    expired = Column(Boolean)
    storage_region = Column(sqlalchemy.Enum(StorageRegion), nullable=False)  # US, EU, ITAR
