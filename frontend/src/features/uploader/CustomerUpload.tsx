...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt < RETRY_ATTEMPTS; attempt += 1) {
    try {
      ...
      lastError = new Error(`${action} failed with status ${response.status}`);
    } catch (error) {
...
function merkleRoot(hashes: string[]): string {
  ...
  let level: Uint8Array<ArrayBufferLike>[] = hashes.map((h) =>
...
export function CustomerUpload() {
  ...
  const addFiles = (files: FileList | File[]) => {
    ...
    setSelectedFiles((currentFiles) => {
      const existingNames = new Set(currentFiles.map((item) => item.file.name));
    ...
    try {
      await runWithConcurrency(selectedFiles, 3, async (item) => {
        try {
          ...
          const { fileHash, chunks } = await buildChunkHashes(
            item.file,
            chunkSizes,
          );
          ...
          await runWithConcurrency(
            ...
            async (chunk) => {
              await uploadChunk(uuid, uploadToken, chunk);
            },
          );
  ...
  return (
    ...
    >
      <div className="upload-panel">
        <p className="upload-eyebrow">Secure upload</p>

        <h1 id="customer-upload-heading">Upload your files</h1>

        <p className="upload-link-id">Upload link ID: {uuid}</p>
